...
<section>
    ...
    <form method="post">
        ...
        <input type="text" id="Attendance_Status" name="Attendance_Status"><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $stmt = $connection->prepare("INSERT INTO attendees(Attendance_ID, Workshop_ID, User_ID, Attendance_Status, Attendance_Date) VALUES (?, ?, ?, ?, ?)"); 
    $stmt->bind_param("issss", $Attendance_ID, $Workshop_ID, $User_ID, $Attendance_Status, $Attendance_Date);

    // Set parameters
    $Attendance_ID = $_POST['Attendance_ID'];
    $Workshop_ID = $_POST['Workshop_ID'];
    $User_ID = $_POST['User_ID'];    
    $Attendance_Status = $_POST['Attendance_Status']; // Corrected variable name
...
<body>
    ...
    <button>search</button></form><h2>Table of attendees</h2></center>
    <table border="5">
        <tr>
            <th>Attendance_ID</th>
            <th>Workshop_ID</th>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $Attendance_ID = $row['Attendance_ID']; // Corrected variable name
                echo "<tr>
                    ...
                    <td>" . $row['Attendance_Date'] . "</td>
                     ...
                     <td><a style='padding:4px' href='delete_attendees.php?Attendance_ID=$Attendance_ID'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_attendees.php?Attendance_ID=$Attendance_ID'>Update</a></td>
                </tr>";
            }
        } else {
